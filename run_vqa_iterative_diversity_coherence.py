...
if args.resume_dir is None:
    ...
    this_results_dir += f"_diversity_coherence"
...
if not is_complete:
    for batch_idx, batch_examples in tqdm(enumerate(dataset.get_batches(IMAGES_CHUNK_SIZE, 
        ...
        for question_idx in tqdm(range(args.max_iterations), desc="running iterative QA"):
            ...
            # Always get coherence metrics for each candidate question
            ...
            # Select best candidate based on coherence metrics and distance

            # First, get coherence scores
            new_scores = []
            parallel_idx = 0
            selected_questions = []
            selected_questions_embeddings = []
            ranking_key = "informativeness_marginal_x_relevance_marginal"
            for batch_sub_idx, (beam_search_questions, beam_search_embeddings) in enumerate(zip(new_questions, embeddings_cand)):
                ...
                candidate_scores = np.array(
                    [candidate_metrics[ranking_key] for candidate_metrics in this_nli_outputs]
                )
            ...
            if question_idx > 0:
                # For later turns, infuse diversity-based ranking by selecting the candidate question that has
                # the largest average cosine distance from previous questions
            
                selected_questions = []
                selected_questions_embeddings = []
                div_scores = []
                for batch_sub_idx, (beam_search_questions, beam_search_embeddings, beam_search_scores) in enumerate(zip(new_questions, embeddings_cand, new_scores)):                    
                    ...
                    max_idx = torch.argmax(combined_score)
                    ...
                    div_scores.append(float(combined_score[max_idx].numpy()))
                
                new_scores = div_scores
...
if worker_index == 0:
    ...
    graph_name = "diversity_coherence"
    if args.n_icl_demonstrations > 0:
