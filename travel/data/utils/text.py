...
def simple_present_to_imperative(nlp: spacy.Language, sentence: str) -> str:
    ...
    # Hack: spacy can't handle if sentence starts with "someone", so correct it here
    if sentence.lower().strip().startswith("someone"):
        subj_idx = 0
    ...
    if subj_idx != -1 and verb is not None and subj_idx < verb.i:
        ...
        imperative_tokens = [token if token.lower() not in ["his", "her", "their"] else "your" for token in imperative_tokens]
        imperative_tokens = [token if token.lower() not in ["himself", "herself", "themself"] else "yourself" for token in imperative_tokens]
