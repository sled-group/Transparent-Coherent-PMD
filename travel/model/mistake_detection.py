...
class MistakeDetectionEvaluator:
    ...
    def check_mistakes(self, detection_threshold: float = 0.5) -> list[bool]:
        """
    ...
    def get_mistake_detection_metrics(self) -> dict[float, dict[str, float]]:
        """
        ...
        for example, outputs in zip(self.examples, self.vqa_outputs):
            ...
            for frame_outputs in outputs:
                ...
                for output in frame_outputs:
                    ...
                    if output.answer_probs[mistake_answer] > detection_threshold:
...
def generate_det_curve(metrics: dict[float, dict[str, float]], save_path: str):
    """
    Generates and saves a PDF of a Detection Error Tradeoff (DET) curve for the metrics returned by `MistakeDetectionEvaluator.get_mistake_detection_metrics()`. A DET curve plots false positive rate (x-axis) versus false negative rate (y-axis) for a space of detection thresholds, and indicates an "ideal" point to set the threshold in the bottom left corner.

    :param metrics: `metrics` object returned by `get_mistake_detection_metrics()`.
    :param save_path: Path to save the PDF of the DET curve.
    """
    # Gather FPR and FNR from metrics
    false_positive_rates = [1.0 - metrics[threshold]['false_positive_rate'] for threshold in metrics]
    false_negative_rates = [1.0 - metrics[threshold]['false_negative_rate'] for threshold in metrics]
    ...
    # Optional: Customize axes for better readability
    ticks = norm.ppf(np.linspace(0.01, 0.99, 9))  # Adjust the range and number of ticks as needed
    tick_labels = np.linspace(0.01, 0.99, 9) * 100  # Convert to percentage for labeling
    plt.xticks(ticks, [f"{label:.1f}%" for label in tick_labels])
    plt.yticks(ticks, [f"{label:.1f}%" for label in tick_labels])
